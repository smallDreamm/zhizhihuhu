...
CREATE TABLE `article`  (
  ...
  `fcontent` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '帖子内容',
  `photo` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '帖子图片',
  `bid` int(11) NOT NULL COMMENT '所属板块id',
...
) ENGINE = InnoDB AUTO_INCREMENT = 209 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '发帖表' ROW_FORMAT = Compact;

-- ----------------------------
-- Records of article
-- ----------------------------
INSERT INTO `article` VALUES (40, 'java集合框架', 'Map\n映射表（也称为关联数组）的基本思想是它维护的是键-值（对）关联，因此你可以使用键来查找值。标准的java类库中包含了Map的几种基本实现。\nHashMap\nMap基于散列表的实现（它实现了HashTable）。插入和查询“键值对”的开销是固定的。可以通过构造器设置容量和负载因子。以调整容器的性能。\nTreeMap\n基于红黑树的实现。查看“键”或者“键值对”时，它们会被排序（次序由Comparable或Comparator决定）。TreeMap的特点在于，所得到的结果是经过排序的。TreeMap是唯一的带有subMap()方法的Map，它可以返回一个子树。', '387c3a86-fd76-48ec-b16a-91a2adc127f54be57d50d7eb0ef7bac518d2e417c5b0.jpg', 1, '2019-05-08 19:22:49', 26, 00000000001);
INSERT INTO `article` VALUES (45, 'FACEBOOK 的系统架构', 'Web 前端是由 PHP 写的。Facebook 的 HipHop [1] 会把PHP转成 C++ 并用 g++编译，这样就可以为模板和Web逻贺业务层提供高的性能。\r\n业务逻辑以Service的形式存在，其使用Thrift [2]。这些Service根据需求的不同由PHP，C++或Java实现（也可以用到了其它的一些语言……）\r\n用Java写的Services没有用到任何一个企业级的应用服务器，但用到了Facebook自己的定制的应用服务器。看上去好像是重新发明轮子，但是这些Services只被暴露给Thrift使用（绝大所数是这样），Tomcat太重量级了，即使是Jetty也可能太过了点，其附加值对Facebook所需要的没有意义。\r\n持久化由MySQL, Memcached [3], Facebook 的 Cassandra [4], Hadoop 的 HBase [5] 完成。Memcached 使用了MySQL的内存Cache。Facebook 工程师承认他们的Cassandra 使用正在减少，因为他们更喜欢HBase，因为它的更简单的一致性模型，以到其MapReduce能力。', 'b644fea0-862b-44ff-b335-5c7f798e6ce22017_9_1_16_57_31_id7_814f253234a906a1d422f3c4fe832fa1.jpg', 24, '2019-05-13 22:47:02', 29, 00000000001);
INSERT INTO `article` VALUES (66, '新配图-修改', '测试hdiohaoshvh mklcmksadmnfkmnasknKNNIKNIKN就没看了奇偶及农我害你和你你可能打击是的纳税方面 看了大家四点上课了能打卡啥都没你看那是多难看到哪门口打死你看吧的奶爸开了的哪款是你的', '3219b13d-9f25-4054-b1d7-73470a8aa870ae1ad9585f84e52765599dc06c9f7891.jpg', 19, '2019-05-26 12:02:17', 28, 00000000001);
...
) ENGINE = InnoDB AUTO_INCREMENT = 187 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '关注表' ROW_FORMAT = Compact;
...
) ENGINE = InnoDB AUTO_INCREMENT = 177 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '收藏表' ROW_FORMAT = Compact;
...
CREATE TABLE `comment`  (
  ...
  INDEX `userid`(`userid`) USING BTREE,
  INDEX `fid`(`fid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 28203 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '评论表' ROW_FORMAT = Compact;
...
CREATE TABLE `enjoy`  (
  `eid` int(11) NOT NULL AUTO_INCREMENT COMMENT '点赞id',
  `userid` int(11) NOT NULL COMMENT '点赞者id',
  `fid` int(11) NOT NULL COMMENT '被点赞帖子id',
  PRIMARY KEY (`eid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 93 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '点赞（欣赏）表' ROW_FORMAT = Compact;

-- ----------------------------
-- Records of enjoy
-- ----------------------------
INSERT INTO `enjoy` VALUES (4, 26, 66);
INSERT INTO `enjoy` VALUES (15, 29, 40);
INSERT INTO `enjoy` VALUES (16, 29, 66);
...
CREATE TABLE `photo_pro`  (
  `fid` int(11) NOT NULL AUTO_INCREMENT COMMENT '相册id',
  `userid` int(11) NOT NULL COMMENT '用户id',
  `name` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '相册名',
  `time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '时间',
  PRIMARY KEY (`fid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 53 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '相册' ROW_FORMAT = Compact;
...
) ENGINE = InnoDB AUTO_INCREMENT = 48 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '板块表' ROW_FORMAT = Compact;
...
CREATE TABLE `tb_photo`  (
  `xid` int(11) NOT NULL AUTO_INCREMENT COMMENT '图片id',
  `fid` int(11) NOT NULL COMMENT '相册id',
  `userid` int(11) NOT NULL COMMENT '用户id',
  `photo` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '图片',
  `time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '时间',
  PRIMARY KEY (`xid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 43 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '图片' ROW_FORMAT = Compact;
...
CREATE TABLE `user`  (
  ...
  `family` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '用户住址',
...
) ENGINE = InnoDB AUTO_INCREMENT = 259 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户表' ROW_FORMAT = Compact;

-- ----------------------------
-- Records of user
-- ----------------------------
INSERT INTO `user` VALUES (26, '马亮', 23, '男', '123', '924818949@qq.com', '四川省广元市青川县', '克己复礼，始于兴趣，勤于专研。', '2018-10-19 12:12:27');
INSERT INTO `user` VALUES (28, '陈意涵', 36, '男', '123', '6755656565@qq.com', '中国台湾', '元气女神', '2019-05-15 09:09:27');
INSERT INTO `user` VALUES (29, '古天乐', 48, '男', '888999', 'maliangnansheng@gmail.com', '中国香港', '无', '2019-05-26 12:50:57');
...
) ENGINE = InnoDB AUTO_INCREMENT = 3513 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '访问记录表' ROW_FORMAT = Compact;
